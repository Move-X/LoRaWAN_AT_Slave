...
#define AT_LINKC      "+LINKC" //Check if the connection with the gateway is still valid thanks to a messagge w/o payload, the gateway respond with the db margin of the last packet send
#define AT_CHANNEL	  "+CHANNEL"

/* Radio Tests Commands */

#ifdef FSK_COMMAND //comandi FSK
#define AT_TRSSI      "+TRSSI" //set the RSSI tone test
#define AT_TTONE      "+TTONE" //set the RF tone test
#endif

#define AT_TTX        "+TTX" //send packets in a PER RF tx
#define AT_TRX        "+TRX" //receive packets in a PER RF rx
#define AT_TTH        "+TTH" //start RF hopping test
#define AT_TCONF      "+TCONF" //set RF LoRa test config
#define AT_CERTIF     "+CERTIF" //set the module in LoRaWan certification in join mode

/* Peripherical Commands */
#define AT_BAT        "+BAT"
#define AT_CW         "+CW"
#define AT_STOPCW     "+STOPCW"
#define AT_CFGSEND    "+CFGSEND"
#define AT_STOPSEND   "+STOPSEND"
...
/**
  * @brief  Time request to the server in UTC format
  * @param  param String parameter
  * @retval AT_OK
  */
...
/**
  * @brief  Add or remove a channel into the node
  * @param  channel_id, frequency_MHz_or_Hz, DR_Max, DR_min
...
//ATEerror_t AT_Net_App_Key_get(const char *param);
//ATEerror_t AT_Nwk_Key_set(const char *param);
...
extern uint32_t number_send_ok;
extern uint32_t number_of_send;
extern uint32_t total_number;
extern uint8_t flag_channel_AU;
...
extern int8_t datarate;
extern LoRaMacRegion_t region;
extern uint32_t payload_length_max;

void Tx_Data_Cmd(void *context);
extern UTIL_TIMER_Object_t Tx_Data;
extern uint32_t send_timer;
extern uint32_t ACK_value;
extern UTIL_TIMER_Object_t LedTimerON;
extern UTIL_TIMER_Object_t LedTimerOFF;
extern UTIL_TIMER_Object_t TimeRequestTimer;
extern uint8_t TimeRequestTimerExp;
extern uint32_t TimeStampSetting;
#ifdef __cplusplus
}
#endif
