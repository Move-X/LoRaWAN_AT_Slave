...
uint8_t second_channel_US = 0xFF;
uint8_t first_channel_US = 0xFF;
...
LoRaMacStatus_t RegionUS915NextChannel( NextChanParams_t* nextChanParams, uint8_t* channel, TimerTime_t* time, TimerTime_t* aggregatedTimeOff )
{
		...
		if( status == LORAMAC_STATUS_OK )
		{
			...
			{
				if ((nextChanParams->Datarate!=DR_4) || (flag_channel_US == 1))
				{
					// Choose randomly on of the remaining channels
					if (first_channel_US == 0XFF || second_channel_US == 0XFF)
					{
						if (first_channel_US == 0XFF)
						{
							*channel = enabledChannels[randr( 0, nbEnabledChannels - 1 )];
							first_channel_US = *channel;
						}
						else//if (second_channel_US == 0XFF)
						{
							*channel = enabledChannels[randr( 0, nbEnabledChannels - 1 )];
							while(*channel == first_channel_US)
							{
							   *channel = enabledChannels[randr( 0, nbEnabledChannels - 1 )];
							}
							second_channel_US = first_channel_US;
							first_channel_US = *channel;
						}
					...
					{
						*channel = enabledChannels[randr( 0, nbEnabledChannels - 1 )];
						while(*channel == first_channel_US || *channel== second_channel_US)
						{
						   *channel = enabledChannels[randr( 0, nbEnabledChannels - 1 )];
						}
						second_channel_US = first_channel_US;
