...
uint8_t second_channel_AU = 0xFF;
uint8_t first_channel_AU = 0XFF;
...
LoRaMacStatus_t RegionAU915NextChannel( NextChanParams_t* nextChanParams, uint8_t* channel, TimerTime_t* time, TimerTime_t* aggregatedTimeOff )
{
    ...
    if( status == LORAMAC_STATUS_OK )
    {
        ...
        {
        	if ((nextChanParams->Datarate!=DR_6) || (flag_channel_AU == 1))
			{
				// We found a valid channel
				// Choose randomly on of the remaining channels
				if (first_channel_AU == 0XFF || second_channel_AU == 0XFF)
				{
					if (first_channel_AU == 0XFF)
					{
						*channel = enabledChannels[randr( 0, nbEnabledChannels - 1 )];
						first_channel_AU = *channel;
					}
					else // if (second_channel_AU == 0XFF)
					{
						*channel = enabledChannels[randr( 0, nbEnabledChannels - 1 )];
						while(*channel == first_channel_AU)
						{
						   *channel = enabledChannels[randr( 0, nbEnabledChannels - 1 )];
						}
						second_channel_AU = first_channel_AU;
						first_channel_AU = *channel;
					}
				...
				{
					*channel = enabledChannels[randr( 0, nbEnabledChannels - 1 )];
					while(*channel == first_channel_AU || *channel== second_channel_AU)
					{
					   *channel = enabledChannels[randr( 0, nbEnabledChannels - 1 )];
					}
					second_channel_AU = first_channel_AU;
