        ...
        switch( payload[macIndex++] )
        {
            ...
            case SRV_MAC_DEVICE_TIME_ANS:
            {
                if( LoRaMacConfirmQueueIsCmdActive( MLME_DEVICE_TIME ) == true )
                {
                    ...
                    SysTime_t ToATime = { 0 }; //MODIFICA

                    ToATime.Seconds = 0; //MODIFICA
					ToATime.SubSeconds = MacCtx.TxTimeOnAir; //MODIFICA
                    ...
                    sysTimeCurrent = SysTimeSub(sysTimeCurrent,ToATime);//MODIFICA
                    sysTime = SysTimeAdd( sysTimeCurrent, SysTimeSub( sysTime, MacCtx.LastTxSysTime ) );

					// Apply the new system time.
                    SysTimeSet( sysTime );
                    //SysTimeSetFirstSynch(sysTime);

                    LoRaMacClassBDeviceTimeAns( );
                    MacCtx.McpsIndication.DeviceTimeAnsReceived = true;
//					#ifndef PRINT_RELEASE
//                    	MW_LOG(TS_OFF, VLEVEL_ALWAYS, "TimeAnsReceived\r\n" );
//					#endif
                    TimeRequestTimerExp = 'f';
                    if (UTIL_TIMER_IsRunning(&TimeRequestTimer) == 1)
                    {
                    	UTIL_TIMER_Stop(&TimeRequestTimer);
                    }
                }
                break;
            }
